...
class Login_model extends CI_Model
{
    ...
    function getMenuList($roleId){
        $this->db->select('tm.id, tm.icon, tm.nama, tm.url, tm.parent_id, tmr.canView, tmr.canCreate, tmr.canUpdate, tmr.canDelete');
        $this->db->from('tbl_menu tm');
        $this->db->join('tbl_matrix_roles tmr', 'tm.id = tmr.MenuId');
        $this->db->where('tmr.roleId', $roleId);
        $this->db->where('tm.isDeleted', 0);
        $this->db->where('tmr.isDeleted', 0);
        $query = $this->db->get();
