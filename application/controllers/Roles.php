...
class Roles extends BaseController
{
    ...
    public function getData() {
        ...
        // debug($postData['param']);
        $data = $this->rm->getRoles($postData);
        echo json_encode($data);
    }

    public function getRowById() {
        $postData = $this->input->post();
        // debug($postData['roleId']);
        $data = $this->rm->getRoleInfo($postData['roleId']);
    ...
    function roleList(){
        ...
        if(!$this->hasListAccess()){
            ...
            $this->global['pageTitle'] = "Role List";
            $pageInfo['param']='7';
            $this->loadViews2("roles/roleList", $this->global, $pageInfo , NULL, 'roles/inputRoles');
        }
    }
    ...
    function addRoles()
    {
        if(!$this->isAdmin()){
            ...
            $status = strtolower($this->security->xss_clean($this->input->post('status')));
            $userInfo = array(
                'role'=>$role, 
                'status'=>$status,
            ...
            $result = $this->rm->addNewRole($userInfo);
            
            if($result > 0){
                $data['message'] = 'Pembuatan Role '.$role.' Berhasil';
                $data['status'] = true;
            } else {
                $data['message'] = 'Pembuatan Role '.$role.' Gagal';
                $data['status'] = false;
            }  
            echo json_encode($data);
        }
    ...
    function deleteRole()
    {
        ...
        else
        {
            ...
            $result = $this->rm->deleteUser($userId, $userInfo);
