...
$(document).ready(function() {
    var table = $('#myTable').DataTable({
        ...
        "columns": [
            ...
            },
            {
                ...
                "width": "15%",
                "render": function (data, type, row) {
                    return '<button class="btn btn-primary btn-edit btn-sm" data-id="'+row.id_guru+'"><i class="fas fa-wrench"></i></button> ' +
                           '<button class="btn btn-danger btn-delete btn-sm" data-id="'+row.id_guru+'"><i class="fas fa-trash"></i></button> ' +
                           '<button class="btn btn-success btn-view btn-sm" data-id="'+row.id_guru+'"><i class="fas fa-eye"></i></button>';
                }
            }
        ]
    });
    ...
    $.ajax({
        url: '<?php echo base_url('guru/addNewTask') ?>',
        type: 'POST',
        data: formData,
    ...
    $('#myTable tbody').on('click', '.btn-edit', function () {
        ...
        $("#editData").modal('show');
        $.ajax({
            url: '<?php echo base_url('guru/getDataById') ?>',
            type: 'POST',
            data: {id: id},
            success: function(response) {
    ...
    $('#myTable tbody').on('click', '.btn-delete', function () {
        ...
        if (confirm("Apakah Anda yakin ingin menghapus data ini?")) {
            $.ajax({
                url: '<?php echo base_url("guru/deleteData") ?>',
                type: 'POST',
    ...
    $.ajax({
        url: '<?php echo base_url("guru/getDataViewById") ?>',
        type: 'POST',
        data: {id: id},
        success: function(response) {
            var data = JSON.parse(response);
            if (data.status) {
                ...
                // Simpan ID guru ke dalam modal
                $('#viewDataModal').attr('data-id', id);

                // console.log("Data yang diterima:", data);
                // console.log("Foto yang diterima:", data.result.foto);
                // console.log("ID Guru yang disimpan di modal:", id);
                // Menampilkan modal
    ...
    var guruId = $('#viewDataModal').attr('data-id');

    console.log("ID Guru saat Cetak CV:", guruId);
    ...
    $.ajax({
        url: '<?php echo base_url("guru/generate_pdf") ?>',
        method: 'POST',
        data: { id_guru: guruId },
        success: function(response) {
            var data = JSON.parse(response);
